...
public class PottedBerryBushBlock extends Block implements IGrowable {
	...
	public static final EnumProperty<BerryType> BERRY_TYPE = MulticraftBlockStateProperties.BERRY_TYPE;
	...
	public void grow(World world, Random rand, BlockPos pos, BlockState state)
	{
		if (!world.isRemote)
		{
			world.setBlockState(pos, Objects.requireNonNull(BlockRegistry.POTTED_BERRY_BUSH.get()).getDefaultState().with(BERRY_TYPE, state.get(BERRY_TYPE)).with(IS_HARVESTED, Boolean.FALSE));
			...
			if (world.getBlockState(pos.down()).getBlock() instanceof HopperBlock)
			{
				...
				world.playSound(null, pos, SoundEvents.ITEM_SWEET_BERRIES_PICK_FROM_BUSH, SoundCategory.BLOCKS, 1.0F, 0.8F + world.rand.nextFloat() * 0.4F);
				world.setBlockState(pos, state.with(IS_HARVESTED, true));
			}
		}
	}
}
